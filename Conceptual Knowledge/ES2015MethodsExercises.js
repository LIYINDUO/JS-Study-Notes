...
Write a function called areAllNumbersFinite 
which accepts an array and returns true if every single value in the array is a finite number, otherwise return false.
...
function areAllNumbersFinite(arr){
    if(arr.find(val => !Number.isFinite(val)) !== undefined){
        return false;
    } else {
        return true;
    }
...
Write a function called convertArrayLikeObject 
...
which accepts a variable number of arguments and returns a new array with all of the arguments that are even numbers.
