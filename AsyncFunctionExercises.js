...
async function hasMostFollowers(...usernames) {
    ...
    let res = await Promise.all(urls);
    let max = res.sort((a,b) => a.followers > b.followers ? -1 : 1)[0];
...
async function starWarsString(id) {
    let personReq = await $.getJSON(`https://swapi.co/api/people/${id}/`);
    let filmReq = await $.getJSON(personReq.films[0]);
    let planetReq = await $.getJSON(filmReq.planets[0]);
    return `${personReq.name} is featured in ${filmReq.title}, directed by ${filmReq.director} and it takes place on ${planetReq.name}`;
}